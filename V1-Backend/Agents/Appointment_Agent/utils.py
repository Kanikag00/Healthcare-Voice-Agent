...
def check_availability(specialty=None, date_str=None, doctor_name=None, time_preference=None):
    ...
    print("requested day: ", requested_day)
    print(day_of_week)
    ...
    for doctor in doctors:
        ...
        print("open ranges - ", open_ranges)

        if open_ranges:
            print("Appending open ranges")
...
def format_slots_for_prompt(slots):
    ...
    for entry in slots:
        ...
        for time_range in entry.get("available_ranges", []):
            lines.append(f"{counter}. Dr. {doctor} on {date} from {time_range[0]} to {time_range[1]}")
            counter += 1
    return "\n".join(lines)
